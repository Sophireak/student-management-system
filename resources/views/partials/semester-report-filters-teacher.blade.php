@props(['classes', 'semesters', 'academicYears' => collect(),
        'routePrefix', 'selectedClassId' => null,
        'selectedSemester' => null, 'selectedYearId' => null])

<div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
    <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">
        Select Semester Report
    </h3>

    <form method="GET"
          action="{{ route($routePrefix . '.semester-report.show') }}">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <div>
                ...
                <select name="semester"
                    ...
                    @foreach ($semesters as $num => $label)
                        <option value="{{ $num }}"
                            {{ $selectedSemester == $num ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
