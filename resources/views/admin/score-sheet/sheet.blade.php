{{-- resources/views/admin/score-sheet/sheet.blade.php --}}
...
<div class="mb-4 flex items-center justify-between flex-wrap gap-2">
    ...
    <a href="{{ route('admin.score-sheet.index', ['class_id' => $class->id]) }}"
...
@else
    <form method="POST"
          action="{{ route('admin.score-sheet.save') }}"
          id="scoreSheetForm">
        @csrf
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200
                    overflow-x-auto">
            <table class="min-w-full text-sm" id="scoreTable">
                ...
                <tbody class="divide-y divide-gray-100" id="scoreBody">
                    ...
                    <tr>
                        ...
                        @foreach ($subjects as $colIndex => $subject)
                            <td class="px-3 py-3 text-center text-sm
                                       font-bold text-blue-700 border-r
                                       border-gray-100"
                                id="avg-col-{{ $colIndex }}">
                                —
                            </td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── Recalculate row + column averages ─────────────────────────
    function recalculate() {
        ...
        for (let r = 0; r < totalRows; r++) {
            ...
            const avgEl = document.getElementById(`avg-row-${r}`);
            if (avgEl) {
        ...
        for (let c = 0; c < totalCols; c++) {
            ...
            const avgEl = document.getElementById(`avg-col-${c}`);
            if (avgEl) {
                avgEl.textContent = count > 0
                    ? (sum / count).toFixed(1) : '—';
            }
