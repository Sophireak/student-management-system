...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Manage all school subjects</p>
    </div>
    <a href="{{ route('admin.subjects.create') }}"
       class="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg transition-colors">
        <i class="ti ti-book-plus text-base"></i> New Subject
    </a>
</div>
    ...
    <table class="min-w-full divide-y divide-gray-100 text-sm">
        <thead class="bg-gray-50">
            <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Max Score</th>
        ...
        <tbody class="divide-y divide-gray-100">
            @forelse ($subjects as $subject)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3 font-mono text-xs text-gray-400">{{ $subject->code ?? '—' }}</td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center flex-shrink-0">
                                <i class="ti ti-book text-green-600 text-sm"></i>
                            </div>
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-4 py-12 text-center">
                        ...
                        <p class="text-gray-400 text-sm">No subjects found.</p>
                        <a href="{{ route('admin.subjects.create') }}"
                           class="mt-3 inline-flex items-center gap-1 text-sm text-green-600 hover:underline">
                            <i class="ti ti-book-plus"></i> Add first subject
                        </a>
                    </td>
                </tr>
            @endforelse
