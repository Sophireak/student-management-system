@extends('layouts.teacher', ['title' => 'Mark Attendance'])

@push('navbar-actions')
    {{-- Class + date chip --}}
    <span class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-50 border border-green-200 text-green-700 text-xs font-semibold rounded-lg">
        <i class="ti ti-school text-sm"></i>
        {{ $attendanceSession->schoolClass->name }}
        · {{ $attendanceSession->session_date->format('M d') }}
    </span>
...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5 mb-4">
        ...
        @if ($attendanceSession->topic)
            <p class="text-xs text-gray-400 mt-1">Topic: {{ $attendanceSession->topic }}</p>
        @endif
    </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-4">
            <div class="px-5 py-3 bg-yellow-50 border-b border-yellow-100">
                <h3 class="text-sm font-semibold text-yellow-800">
                    Mark Attendance
                    <span class="font-normal text-yellow-600">({{ $unmarkedEnrollments->count() }} students pending)</span>
                </h3>
            </div>

            <form method="POST"
                  action="{{ route('teacher.attendance-sessions.attendance.store', $attendanceSession) }}"
                  novalidate>
                @csrf
                    ...
                    <div class="px-5 py-3 border-b border-gray-100 last:border-0">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                @foreach (['present', 'absent', 'late', 'excused'] as $status)
                                    <label class="cursor-pointer">
                                        ...
                                        <span class="px-3 py-1 text-xs rounded-full border border-gray-200 text-gray-500 capitalize peer-checked:font-semibold
                                            {{ match($status) {
                            ...
                            <input type="text"
                                   ...
                                   class="text-xs border border-gray-200 rounded px-2 py-1 w-36 focus:outline-none focus:ring-1 focus:ring-green-400 text-gray-600">
                        </div>
                    </div>
                @endforeach
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-5 py-3 bg-gray-50 border-b border-gray-100">
                <h3 class="text-sm font-semibold text-gray-700">
                    Marked Records
                    @if ($isFullyMarked)
                        <span class="ml-2 px-2 py-0.5 text-xs bg-green-100 text-green-700 rounded-full">Complete</span>
                    @endif
                </h3>
            </div>
            ...
            @foreach ($attendanceSession->attendances as $record)
                <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100 last:border-0">
                    ...
                    <form method="POST"
                          action="{{ route('teacher.attendance-sessions.attendance.update', [$attendanceSession, $record]) }}"
                          class="flex items-center gap-2">
                        @csrf
                        @method('PATCH')

                        <select name="status"
                                class="text-xs border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-green-400
                                    {{ match($record->status) {
                        ...
                        <input type="text" name="notes" value="{{ $record->notes }}" placeholder="Note"
                               class="text-xs border border-gray-200 rounded px-2 py-1 w-28 focus:outline-none focus:ring-1 focus:ring-green-400">
