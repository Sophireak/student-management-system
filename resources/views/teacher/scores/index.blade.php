    ...
    <form method="GET"
        ...
        <div class="flex-1 min-w-48">
            ...
            @if ($classes->count() === 1)
                ...
                <input type="hidden" name="class_id" id="sel-class" value="{{ $singleClass->id }}">
                <div class="relative w-full border border-gray-200 bg-gray-50 
                            rounded-lg pl-9 pr-8 py-2 text-sm text-gray-700">
                    <i class="ti ti-building absolute left-3 top-1/2 -translate-y-1/2 
                              text-gray-400 text-sm"></i>
                    <span class="font-semibold">{{ $singleClass->name }}</span>
                    <span class="text-gray-400">({{ $singleClass->grade->name }})</span>
                    <i class="ti ti-lock absolute right-3 top-1/2 -translate-y-1/2 
                              text-gray-300 text-xs" title="Your assigned class"></i>
                </div>
            @else
                <div class="relative">
                    <i class="ti ti-building absolute left-3 top-1/2 -translate-y-1/2 
        ...
        <div class="flex-1 min-w-48">
            ...
            <div class="relative">
                <i class="ti ti-calendar-stats absolute left-3 top-1/2 -translate-y-1/2 
                          text-gray-400 text-sm"></i>
                <select name="period" id="sel-period" required
                    ...
                    <optgroup label="Semester">
                        <option value="semester_1" 
                                {{ ($selectedPeriod ?? '') === 'semester_1' ? 'selected' : '' }}>
                            {{ \App\Helpers\AcademicCalendar::semesterLabel(1) }}
                        </option>
                        <option value="semester_2" 
                                {{ ($selectedPeriod ?? '') === 'semester_2' ? 'selected' : '' }}>
...
@if (! $hasFilter)
    <div class="bg-white rounded-xl border border-gray-200 px-5 py-16 text-center">
        ...
        <p class="text-gray-500 text-sm">Select a period above to view subjects.</p>
    </div>
...
@else
    ...
    {{-- Lock Status (info only for teacher) --}}
    <div class="flex items-center gap-2 text-sm bg-white rounded-xl border border-gray-200 
                px-5 py-3 mb-5">
        @if ($isLocked)
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-3 border-b border-gray-200 flex items-center justify-between">
            ...
            <p class="text-xs text-gray-400 hidden sm:block">Tap to enter scores</p>
        </div>
        ...
        <div class="divide-y divide-gray-100">
            @foreach ($subjects as $subject)
                ...
                <a href="{{ route('teacher.scores.input', array_merge(request()->query(), ['subject_id' => $subject->id])) }}"
                   class="flex items-center gap-3 px-5 py-4 hover:bg-gray-50 transition-colors group">

                    {{-- Icon --}}
                    <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0
                                {{ $isDone ? 'bg-green-100 text-green-600' 
                                    : ($isEmpty ? 'bg-gray-100 text-gray-400' : 'bg-yellow-100 text-yellow-600') }}">
                    ...
                    {{-- Progress (Desktop) --}}
                    <div class="hidden sm:flex flex-col items-end gap-1 w-32">
                        ...
                        <div class="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div class="h-full rounded-full transition-all
                                        {{ $isDone ? 'bg-green-500' 
                                            : ($isEmpty ? 'bg-gray-300' : 'bg-yellow-500') }}"
                    ...
                    <span class="hidden md:inline-flex text-xs font-medium px-2 py-1 rounded-md
                                 {{ $isDone ? 'bg-green-50 text-green-700' 
                                     : ($isEmpty ? 'bg-gray-50 text-gray-500' : 'bg-yellow-50 text-yellow-700') }}">
                        {{ $isDone ? 'Complete' : ($isEmpty ? 'Not Started' : 'In Progress') }}
                    </span>

                    {{-- Mobile: Just count --}}
                    <span class="sm:hidden text-xs font-medium text-gray-500">
                        {{ $completed }}/{{ $totalStudents }}
                    </span>

                    <i class="ti ti-chevron-right text-gray-300 group-hover:text-gray-500 transition-colors flex-shrink-0"></i>
                </a>
            @endforeach
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function trySubmit() {
        if (selClass && selClass.value && selPeriod && selPeriod.value) {
            form.submit();
        }
    }

    // Only bind listener if class is a SELECT (not hidden input)
    if (selClass && selClass.tagName === 'SELECT') {
