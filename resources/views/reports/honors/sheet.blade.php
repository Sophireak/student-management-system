    ...
    <div>
        <a href="{{ route($routePrefix . '.reports.honors.index') }}"
           class="{{ $linkClass }}">
            ...
            {{ $isAdmin ? '← Honors Report' : 'Honors Report' }}
        </a>
    ...
    <form method="GET"
        ...
        <div>
            ...
            <select name="class_id"
                    class="border border-gray-300 {{ $selRadius }} px-2 py-1.5 text-sm
...
@if ($rows->isEmpty())
    <div class="bg-white {{ $isAdmin ? 'rounded-lg' : 'rounded-2xl' }} shadow-sm border border-gray-200 px-5 py-8
...
@else
    ...
    {{-- Full honors table --}}
    <div class="bg-white {{ $isAdmin ? 'rounded-lg' : 'rounded-2xl' }} shadow-sm border border-gray-200 overflow-hidden print:shadow-none print:border-0">
